...
from ETFOptimizer.items import EtfItemLoader
...
class ExtraetfSpider(scrapy.Spider):
    ...
    # TODO we should do some double checking with expected strings!!! and check for empty!!!
    url_to_asset = {
        2: "Aktien",
        3: "Anleihen",
        4: "Rohstoffe",
    ...
    def parse(self, response, **kwargs):
        ...
        for asset_id in self.url_to_asset.keys():
            link = base_link + str(asset_id)
            yield Request(link, callback=self.page_walk_run, cb_kwargs=dict(category=self.url_to_asset[asset_id],
                                                                            callback=self.parse_category))


        for asset_id in self.url_to_asset.keys():
            if asset_id == 2 or asset_id == 3 or asset_id == 1160:
                for region_id in self.url_to_region.keys():
                    link = base_link + str(asset_id) + "&region=" + str(region_id)
                    yield Request(link, callback=self.page_walk_run, cb_kwargs=dict(
                        subcategory=self.url_to_region[region_id], callback=self.parse_subcategory))

                for country_id in self.url_to_country.keys():
                    link = base_link + str(asset_id) + "&land=" + str(country_id)
                    yield Request(link, callback=self.page_walk_run, cb_kwargs=dict(
                        subcategory=self.url_to_country[country_id], callback=self.parse_subcategory))

                if asset_id == 2:
                    for sector_id in self.url_to_sector.keys():
                        link = base_link + str(asset_id) + "&sector=" + str(sector_id)
                        yield Request(link, callback=self.page_walk_run, cb_kwargs=dict(
                        subcategory=self.url_to_sector[sector_id], callback=self.parse_subcategory))
